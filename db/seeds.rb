...
user = Users::Create.new.call(name: 'Shane Davies', email: 'sdavies@loft47.com', password: 'password')

[
  [:business_value, 'Customer Benefit', 'What is the need and urgency of this feature? How often does a customer encounter this issue?', 'Minor annoyance or small benefit; happens rarely; not urgent.', 'Solves a major customer pain or provides a significant benefit; Occurs often; needs a solution as soon as possible.'],
  ...
  Measure.create!(section: section, name: name, hint: hint, start_hint: start_hint, end_hint: end_hint, position: index, required: false)
end

feature = Feature.create(user: user, name: 'SMS Notification', description: 'Allow users to opt into SMS notifcation', stakeholder: 'users', documentation: 'see twillio api for details', required_at: '2017-12-15')
Measure.all.each do |measure|
  Score.create!(user: user, feature: feature, measure: measure, value: 3)
end
